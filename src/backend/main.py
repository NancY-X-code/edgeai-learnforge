...
def prefetch_video_assets(video_id: str, storage_dir: str):
    ...
    try:
        ...
        # We use max_workers=2 for background generation to keep CPU/GPU loads stable
        with ThreadPoolExecutor(max_workers=2) as executor:
            for i in range(len(topics)):
                # 1. Notes MUST run sequentially first, as they build the underlying knowledge schema cache
                try:
                    generate_notes_for_single_topic(video_id, i, storage_dir)
                except Exception as e:
                    print(f"[LearnForge Prefetch] Notes failed for topic {i}: {e}")
                
                # 2. Flashcards, Quiz, and Graph can run concurrently since they depend on the same notes schema or transcript segments
                def run_flashcards():
                    try:
                ...
                def run_graph():
                    try:
                        generate_graph_for_single_topic(video_id, i, storage_dir)
                    except Exception as e:
                        print(f"[LearnForge Prefetch] Graph failed for topic {i}: {e}")
                
                future_fc = executor.submit(run_flashcards)
                future_qz = executor.submit(run_quiz)
                future_gr = executor.submit(run_graph)
                
                future_fc.result() # Wait for all before moving to the next topic
                future_qz.result()
                future_gr.result()
                
        print(f"[LearnForge Timing] Completed background pre-generation for video {video_id} in {time.time() - t0_prefetch:.2f}s.")
    except Exception as e:
...
async def process_transcript(req: ProcessRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        print(f"[LearnForge Timing] Topic segmentation took {time.time() - t0_seg:.2f}s")
        ...
        # Use ThreadPoolExecutor (max 4 workers) to safely process topics concurrently
        # This drastically reduces foreground API blocking time
        topics_enriched = []
        ...
        print(f"[LearnForge Timing] Concept density processing took {time.time() - t0_density:.2f}s")
    ...
    try:
        ...
        print(f"[LearnForge Timing] FAISS Indexing took {time.time() - t0_index:.2f}s")
    except Exception as e:
        print(f"[LearnForge API] FAISS indexing error: {e}")
        raise HTTPException(status_code=500, detail="Index creation failed.")

    print(f"[LearnForge Timing] Total /process request took {time.time() - t0_process:.2f}s")
...
async def chat_with_tutor(request: Request):
    try:
        ...
        response_text = generate_chat_response(video_id, topic_title, question, history, STORAGE_DIR)
        return {"response": response_text}
    except Exception as e:
        print(f"[LearnForge API] Chat error: {e}")
